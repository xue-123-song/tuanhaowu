export default (message, callback) => {
    ...
    function share(id, msg, callback) {
        ...
        var _share = function() {
            service.send(msg, function() {
                window.plus.nativeUI.toast("分享到\"" + service.description + "\"成功！");
                callback && callback(true);
            }, function(e) {
                window.plus.nativeUI.toast("分享到\"" + service.description + "\"失败！");
                callback && callback(false);
            })
        };
    ...
    function openSystem(msg, callback) {
        if(window.plus.share.sendWithSystem) {
            window.plus.share.sendWithSystem(msg, function() {
            //TODO 系统分享暂不支持回调
            //callback && callback(true);
            }, function() {
            //TODO 系统分享暂不支持回调
