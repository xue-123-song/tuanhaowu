...
import { Form, Input, Button, Avatar, AutoCenter, Divider, NavBar, Toast } from 'antd-mobile';
...
export default (props) => {
    ...
    const onFinish = async (v) => {
        ...
        let user = await login(v.account, v.password);
        if(user != null) {
            props.setUser(user);
            props.setRole("团长");
    ...
    return (
        <div style={{flex: 1}}>
            <NavBar backArrow={false} right={<span style={{ fontSize: 14 }} onClick={()=>{ props.setRole('游客'); navigate('/', { replace: true }) }}>以游客身份使用<RightOutline /></span>} />
