...
import { queryInfo } from '../services/userService';
...
export default (props) =>{
    ...
    useEffect(() => {
        const getData = async() => {
            let userInfo = await queryInfo(props.user.userid);
            ...
            <div style={{color: '#ededed', height: '10px'}}></div>
            {
...

                {
                    ...
                    <List header=''>
                        ...
                        <List.Item prefix={<Badge content='5'><MessageOutline color='green' /></Badge>} onClick={() => {navigate("/message", {})}}>
                            我的消息
                        </List.Item>
